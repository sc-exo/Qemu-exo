...
static void virtio_blk_req_complete(VirtIOBlockReq *req, unsigned char status)
{
    ...
    // printf("virtio_blk_req_complete is req->elem index is %u,len is %lu\n",req->elem.index,req->in_len);
...
static int virtio_blk_handle_request(VirtIOBlockReq *req, MultiReqBuffer *mrb)
{
    ...
    {
        // printf("virtio_blk_handle_request is flush\n");
...
static void virtio_blk_device_realize(DeviceState *dev, Error **errp)
{
    ...
    for(i=0;i<4;i++)
    {
        __sys_io_uring_register(bs->ring[i]->ring_fd, IORING_REGISTER_BPF_vqaddr, &vq_addr , 0);
    }
