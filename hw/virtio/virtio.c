...
static void virtio_queue_split_set_notification(VirtQueue *vq, int enable)
{
    ...
    if (virtio_vdev_has_feature(vq->vdev, VIRTIO_RING_F_EVENT_IDX)) {
        // printf("---- shadow_avail_idx is %u, last_avail_idx is %u, enable is %d\n", vq->shadow_avail_idx,vq->last_avail_idx,enable);
...
void virtio_bpf_uring_fd_register(VirtQueue *vq, VirtIODevice *vdev)
{
    ...
    for(i=0;i<4;i++)
    {
        __sys_io_uring_register(bs->ring[i]->ring_fd, IORING_REGISTER_BPF_vqfd, &vq_fd , 0);
    }
    // __sys_io_uring_register(bs->ring[0]->ring_fd, IORING_REGISTER_BPF_vqfd, &vq_fd , 0);
