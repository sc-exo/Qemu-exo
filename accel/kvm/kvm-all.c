...
int kvm_init_vcpu(CPUState *cpu, Error **errp)
{
    ...
    // printf("kvm_init_vcpu id is %lu\n",kvm_arch_vcpu_id(cpu));
    fd = KVM_Router_fd;
    kvm_vm_ioctl(s, KVM_SET_ROUTER_FD, &fd);
    if(kvm_arch_vcpu_id(cpu)==0)
    {
        kvm_vm_ioctl(s, KVM_GET_ROUTER_ADDR, &addr);
        printf("kvm_init_vcpu router addr is %lx\n",addr);
        
        KVM_Router_addr = addr;
    }
    enable = 1;
    kvm_vm_ioctl(s, KVM_GET_ROUTER_ENABLE, &enable);    

    ring_fd = KVM_Ring_fd;
    kvm_vm_ioctl(s, KVM_SET_RING_FD, &ring_fd);
